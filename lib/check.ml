...
(* --- 8. Undefined constant references --- *)
...
(* --- 11. Initial transition scope validation --- *)
...
(* --- 12. Typed element mismatch detection --- *)
...
let widen_types (a : Ast.type_name) (b : Ast.type_name) =
  if a = b then Some a
    ...
    | Cat_int wa, Cat_int wb ->
        (* Widen to larger integer. Pick whichever has the larger width. *)
        if wa >= wb then Some a else Some b
...
let state_machine config (sm : Ast.def_state_machine) =
  ...
  | Some members ->
      ...
      in
      let initial =
        validate_sm_initial ~sm_name ~sm_loc:sm.sm_name.loc members
      in
      let state_initial =
        List.concat_map
          (fun ann ->

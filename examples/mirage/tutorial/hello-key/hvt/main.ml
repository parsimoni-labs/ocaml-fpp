...
let unikernel__hello =
  let doc = Cmdliner.Arg.info ~doc:"hello" ["hello"] in
  Mirage_runtime.register_arg Cmdliner.Arg.(value & opt string "Hello World!" doc)

let start = lazy (Unikernel.start ~hello:(unikernel__hello ()))
...
let () =
  let t =
    ...
    let* () = Lwt.return (Mirage_runtime.(with_argv (runtime_args ()) "Solo5HelloKey" (Mirage_bootvar.argv ()))) in
    ...
    Mirage_runtime.set_name "Solo5HelloKey";

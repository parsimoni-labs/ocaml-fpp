...
module type Conduit_tls = Conduit_mirage.S
...
module Conduit_tls = Conduit_tls.Make(Stackv4v6)
module Cohttp_server = Cohttp_mirage.Server.Make(Conduit_tls)
...
let start = lazy (
  ...
  let* conduit_tls = Conduit_tls.start stackv4v6 in
  let cohttp_server = Cohttp_server.listen conduit_tls in

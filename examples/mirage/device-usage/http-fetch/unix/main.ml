...
let start = lazy (
  ...
  let* resolver_unix = Resolver_unix.connect () in
  let* conduit_tcp = Conduit_tcp.start stackv4v6 in
  let* cohttp_client = Cohttp_client.connect resolver_unix conduit_tcp in
  Unikernel.start cohttp_client)
...
let () =
  let t =
    ...
    let* () = Lwt.return (Mirage_runtime.(with_argv (runtime_args ()) "UnixHttpFetch" (Mirage_bootvar.argv ()))) in
    ...
    Mirage_runtime.set_name "UnixHttpFetch";

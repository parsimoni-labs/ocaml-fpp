...
let netif__0 =
  let doc = Cmdliner.Arg.info ~doc:"0" ["netif-0"] in
  Mirage_runtime.register_arg Cmdliner.Arg.(value & opt string "" doc)
let ipv6__no_init =
...
let connect = lazy (
  ...
  let* ipv6 = Ipv6.connect ~no_init:(ipv6__no_init ()) netif ethernet in

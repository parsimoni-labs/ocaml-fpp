...
module Make
  ...
  let c ~write net =
    let open Lwt.Syntax in
    let* tcp = Tcp.connect ~write net in
    Lwt.return { net; tcp }
end|}
...
let test_all_leaves_bound () =
  check_output ~msg:"all leaves bound, no functor"
    (render_topo
       {|
    port P
    @ ocaml.functor Srv.Make
...
let test_multi_group () =
  check_output ~msg:"two groups produce independent types and functions"
    (render_topo
       {|
    port P
...
let test_empty_topology () =
  check_output ~msg:"empty topology emits unit type"
    (render_topo {|
    topology Empty {
    } |})
    {|(* Generated by ofpp to-ml from topology Empty *)
...
let test_bound_nonleaf () =
  check_output ~msg:"bound non-leaf gets module alias + connect with deps"
    (render_topo
       {|
    port P
    port Dep
...
module Make = struct
  ...
  type t = { stack : Stack.t; srv : Srv.t; }

  let w () =
    let open Lwt.Syntax in
    let* stack = Stack.connect () in
    let* srv = Srv.connect stack in

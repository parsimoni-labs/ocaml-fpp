(** Tests for {!Fpp.Gen_ml}: FPP to OCaml code generation.
    ...
    functor-application mode, default functor convention, topology import,
...
module Make
  (C : sig include C val start : unit -> t end)
  (B : sig include B val start : C.t -> t end)
...
module Make
  (Logger : sig include LOGGER val connect_device : unit -> t end)
  (Sensor : sig include SENSOR val connect_device : Logger.t -> t end) = struct
  type t = { logger : Logger.t; sensor : Sensor.t; }

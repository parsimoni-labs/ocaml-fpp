...
let test_bound_leaf_instance () =
  check_output ~msg:"qualified leaf emits alias, auto-inits in connect"
    (render_topo
       {|
    port P
    active component Eth {
    ...
    instance net: Net base id 0x100
    instance eth: Eth base id 0x200
    instance data: Htdocs.Data base id 0x300
    topology T {
      instance net
      instance eth
      instance data
...
let test_mixed_bound_unbound () =
  check_output
    ~msg:"qualified leaf + unbound leaves, functor params only for unbound"
    (render_topo
       {|
    port P
...
let test_all_leaves_bound () =
  check_output ~msg:"all leaves qualified, top-level lazy bindings"
    (render_topo
       {|
    port P
    active component Srv {
...
let test_bound_nonleaf () =
  check_output ~msg:"qualified leaf gets alias, top-level lazy with deps"
    (render_topo
       {|
    port P
    port Dep
...
let test_runtime () =
  check_output ~msg:"runtime labelled args inject thunk calls"
    (render_topo
       {|
    port P
    ...
    instance sock: My.Socket base id 0x100
    instance srv: Srv base id 0x200
    topology T {
      instance runtime
      instance sock
...
let test_mli_import_sig_path () =
  check_output
    ~msg:"import interface derives sig path for .mli module type constraint"
    (render_topo_mli
       {|
    module Net {
      interface S {

...
let gen_ml_types_only ppf merged_tu per_file =
  let file_topos =
    List.filter_map
      (fun (file, tu) ->
        let topos = Fpp.topologies tu in
  ...
  let file_topos =
    List.filter
      ...
      file_topos
  in
  if file_topos <> [] then begin
    Fmt.pf ppf "(* Module types generated by ofpp to-ml *)@.@.";
    List.iter
      (fun (file, topos) ->
        let name = module_name_of_file file in
        Fmt.pf ppf "@[<v>module %s = struct" name;
        ignore (Fpp.Gen_ml.pp_module_types_raw merged_tu topos ppf);
        Fmt.pf ppf "@]@.end@.@.")
      file_topos
  end
...
let gen_ml_for_tu ppf tu ~per_file ~sm_name ~topologies ~types_only =
  if types_only then gen_ml_types_only ppf tu per_file
...
let to_ml ~output ~sm_name ~topologies ~types_only files =
  ...
  | Some per_file ->
      ...
      gen_ml_for_tu ppf tu ~per_file ~sm_name ~topologies ~types_only;
      Fmt.flush ppf ();
